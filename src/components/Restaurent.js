    ...
    return (
        <>
            <div className='grid grid-cols-4 w-11/12'>
                {props?.restaurantsList?.filter((data)=> data?.cuisine[0]?.name.includes(props?.menu) || data?.cuisine[1]?.name.includes(props?.menu) || data?.cuisine[2]?.name.includes(props?.menu)).map((data) => {
                    return <>
                        <Link to="/details" state={{data:data}}><div  className="max-w-sm  overflow-hidden  mt-4">
                            ...
                            <div className="px-4 py-2">
                                ...
                                <img src={rating} className='w-5 h-5 rounded-full' alt=""/>
